    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-foreground">{t('userManagement')}</h1>
            <p className="text-muted-foreground">{t('viewCreateManageUsers')}</p>
          </div>
        </div>
          ...
          <CardContent className="p-4">
            ...
            {hasActiveFilters && (
              <div className="mt-3 flex items-center gap-2 text-sm text-muted-foreground">
                ...
                {status !== 'all' && <Badge variant="secondary">{t('status')}: {status}</Badge>}
                {roleId !== 'all' && (
                  <Badge variant="secondary">
                    {t('role')}: {rolesData?.find((r) => r.id === parseInt(roleId))?.name}
                  </Badge>
                )}
              </div>
            )}
          ...
          <Table isLoading={isLoading}>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="hidden lg:table-cell">{t('role')}</TableHead>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-sm text-muted-foreground md:hidden truncate">{user.email}</p>
                        </div>
                      </div>
                    </TableCell>
